...
int brushsize = 5; // brush or eraser size

void SDL_RenderDrawCircle(SDL_Renderer* renderer, int x, int y, int radius){
  for(int angle = 0; angle < 360; angle++){
    float radian = angle * M_PI / 180.0f;
    int dx = x + (int)(radius * cos(radian));
    int dy = y + (int)(radius * sin(radian));
...
void FillRectangle(SDL_Renderer* renderer, int x, int y, int w, int h){
    ...
    while (SDL_PollEvent(&event)){
      if (event.type == SDL_QUIT){
        ...
        if(drawing){
          SDL_SetRenderTarget(renderer, canvasTexture); // use canvas for painting
          SDL_SetRenderDrawColor(renderer, currentColor.r, currentColor.g, currentColor.b, currentColor.a);
          FillCircle(renderer, event.motion.x, event.motion.y, brushsize);
